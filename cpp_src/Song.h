...
struct TChannelVariables
{
	...
	bool isInstrumentEnvelopeLooped;		// Instrument Envelope flag for looping, used to trigger Volume Slide if enabled
	bool isVolumeOnlyEnabled;				// AUDC, bit 4, forcing Volume Only output in the Channel when enabled
	char channelNote;						// Maximum set by PATTERN_NOTE_COUNT, currently set to 120 (Channel)
	BYTE channelVolume;						// AUDC, bits 0-3, Volume level could be 0-15 inclusive (Channel)
	BYTE channelTimbre;						// AUDC, bits 5-7, unique tones and noises generated by the POKEY Soundchip (Channel)
	BYTE channelAudctl;						// AUDCTL, bits from all 4 POKEY Channels combined during playback (Channel)
	TInstrumentV2* pInstrument;				// Pointer to currently Active Instrument in Channel
	TEnvelopeVariables instrumentEnvelope;	// Variables used to keep track of the Instrument Envelope Offsets and Speeds during playback
	...
	BYTE instrumentVolumeSlide;				// Volume Slide parameter, set using the Axx Command, the Instrument Volume Slide Parameter will take priority if enabled
	char finetuneOffset;					// Finetune offset, set using the ?xx Command, signed values are expected, thus $01 will add 1, and $FF will subtract 1, etc
};
...
public:
	...
	void ProcessNote(BYTE note, BYTE channel, TChannelVariables* pVariables);
	void ProcessInstrument(BYTE instrument, BYTE channel, TChannelVariables* pVariables);
	void ProcessVolume(BYTE volume, BYTE channel, TChannelVariables* pVariables);
	void ProcessEffect(TEffect* effect, BYTE channel, TChannelVariables* pVariables);
