...
int CRmtView::MouseAction(CPoint point,UINT mousebutt,short wheelzDelta=0)
{
	...
	if (rec.PtInRect(point))
	{
		...
		if (mousebutt & MK_LBUTTON)
		{
			BOOL r=g_Song.InfoCursorGotoSongname(point.x-INFO_X);
			if (r) SCREENUPDATE;
		}
		return 6;
	}
	...
	if (rec.PtInRect(point))
	{
		...
		if (mousebutt & MK_LBUTTON)
		{
			BOOL r=g_Song.InfoCursorGotoSpeed(point.x-(INFO_X+13*8));
			if (r) SCREENUPDATE;
		}
		return 6;
	}
	...
	if (rec.PtInRect(point))
	{
		...
		if (wheelzDelta!=0)
		{
			...
			if (wheelzDelta<0) r=g_Song.OctaveDown();
			if (r) SCREENUPDATE;
		}
		return 6;
	}
	...
	if (rec.PtInRect(point))
	{
		...
		if (wheelzDelta!=0)
		{
			...
			if (wheelzDelta<0) r=g_Song.VolumeDown();
			if (r) SCREENUPDATE;
		}
		return 6;
	}
...
Instrument_Select_Dialog:
		...
		if (wheelzDelta!=0)
		{
			...
			if (wheelzDelta<0) g_Song.ActiveInstrNext();
			SCREENUPDATE;
		}
		return 6;
	}
